  ...
  async.series([
      function(callback){
          OAuthUsersModel.findOne({username: username}, function(err, user){
              ...
              if (user) {
                  utils.debug('found existing user');
                  callback('Sorry, username: "'+username+'" already exists');
              }else{
                  utils.debug('no user found');
                  callback();
              }
          });
      },
      function(callback){
          utils.debug('new user being added.');
  ...
  async.series([
      function(callback){
          OAuthUserRolesModel.findOne({userId: userId, clientId: clientId}, function(err, user){
              if (err) {
                  utils.debug('error in find One User Role '+err);
                  callback(err);
              }
              if (user) {
                  utils.debug('found existing user role');
      ...
      function(callback){
          utils.debug('new user role being added.');
